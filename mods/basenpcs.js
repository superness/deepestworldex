...
setTimeout(() => {
    FauxNPCs.addNPC('ayy', npcSpots.ayy.x, npcSpots.ayy.y, npcSpots.ayy.z, () => {console.log('click ayyylien'); Dialogs.ShowDialog('ayy'); junkCollector()})
    FauxNPCs.addNPC('vampire', npcSpots.vampire.x, npcSpots.vampire.y, npcSpots.vampire.z, () => {console.log('click vampire'); Dialogs.ShowDialog('vampire'); gearCurator()})
    FauxNPCs.addNPC('orc', npcSpots.orc.x, npcSpots.orc.y, npcSpots.orc.z, () => {console.log('click orc'); Dialogs.ShowDialog('orc'); materialCollector()})
    ...
    Dialogs.AddDialog('ayy', npcSpots.ayy.x + 0.3, npcSpots.ayy.y - 0.5, 'Take junk', junkCollector)
    Dialogs.HideDialog('ayy')

    Dialogs.AddDialog('orc', npcSpots.orc.x + 0.3, npcSpots.orc.y - 0.5, 'Take materials', materialCollector)
...
async function materialCollector() {
    ...
    for(let box of nearbyBoxes) {
        ...
        for(let i of box.storage) {
            ...
            if(getIsItemInGearCache(i)) {
                dw.log(`sending item in cache to player ${idx}`)
                dw.moveItem('storage', idx, 'bag', dw.c.bag.findIndex(i => i == null), box.id, null, null)
                await sleep(100)
            }
        }
...
async function disenchanter() {
    ...
    for(let item of dw.c.bag) {
        ...
        if(!getDoesMatchAnyGearSetFilter(item) && !dw.md.items[item.md]?.mat && !dw.md.items[item.md]?.s) {
            ...
            console.log(`Disenchanting ${idx} at ${enchantingDevice.id}`)
            dw.disenchant(enchantingDevice.id, idx)
