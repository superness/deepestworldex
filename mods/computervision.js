...
class ComputerVision {
    ...
    static getMonsterBattleScore(monster, c, useFullHp = false) {
        ...
        return Math.sqrt(hpUse * (ComputerVision.getMonsterDmg(monster, c) - (ComputerVision.getSkillDamage(0) * 0.5)))
    ...
    static getMonsterDmg(monster, c) {
        ...
        return Math.max(1, (dmg * (monster.md.includes('Pow') ? 1.25 : 1))) * 1.1
    }
        ...
        function* yieldVisionGridUpdatesOnOldSpots() {
            while (true) {
                ...
                for (let spot of visionGridEx) {
                    ...
                    let spotInfo = ComputerVision.getSpotInfo(x, y, e.data.monsters, e.data.nonTraversableEntities, e.data.c, e.data.gearTesting, e.data.optimalMonsterRange, e.data.optimalMonsterRangeBuffer, e.data.targetZoneLevel, e.data.targetId, e.data.nearMonsterUnsafeRadius)
